...
class UserGenerationWebService(user_generation_service.UserGenerationService):
    def generate_user(self) -> user.UserModel:
        ...
        return self.__map_service_user_to_domain__(service_user)
    
    @staticmethod
    def __map_service_user_to_domain__(service_user: UserServiceModel) -> user.UserModel:
        return user.UserModel(
            ...
            location= user.LocationEntity(
                address_line_one= str(service_user.location['street']['number']) + ' ' + service_user.location['street']['name'],
